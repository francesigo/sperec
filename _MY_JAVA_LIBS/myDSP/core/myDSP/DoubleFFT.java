 ...
 * 
 */
public strictfp class DoubleFFT {
    ...
    private static final double PI = 3.14159265358979311599796346854418516f;

    private static final double TWO_PI = 6.28318530717958623199592693708837032f;

    /**
     * Creates new instance of DoubleFFT.
     * 
     * @param n
     *            size of data
     */
    public DoubleFFT(int n) {
        if (n < 1) {
    ...
    public void realInverse(double[] a, int offa, boolean scale) {
        ...
        switch (plan) {
        case SPLIT_RADIX:
            a[offa + 1] = (double)(0.5 * (a[offa] - a[offa + 1]));
            a[offa] -= a[offa + 1];
            if (n > 4) {
                rftfsub(n, a, offa, nc, w, nw);
        ...
        for (k1 = 1; k1 <= nf; k1++) {
            ...
            for (j = 1; j <= ipm; j++) {
                ...
                for (ii = 4; ii <= idot; ii += 2) {
                    ...
                    arg = fi * argld;
                    int idx = i + twon;
                    wtable[offw + idx - 1] = (double)Math.cos(arg);
                    wtable[offw + idx] = (double)Math.sin(arg);
                }
                if (ip > 5) {
                    int idx1 = i1 + twon;
                    int idx2 = i + twon;
        ...
        for (k1 = 1; k1 <= nf; k1++) {
            ...
            for (j = 1; j <= ipm; j++) {
                ...
                for (ii = 4; ii <= idot; ii += 2) {
                    ...
                    arg = fi * argld;
                    int idx = i + twon;
                    wtable[idx - 1] = (double)Math.cos(arg);
                    wtable[idx] = (double)Math.sin(arg);
                }
                if (ip > 5) {
                    int idx1 = i1 + twon;
                    int idx2 = i + twon;
        ...
        for (k1 = 1; k1 <= nfm1; k1++) {
            ...
            for (j = 1; j <= ipm; ++j) {
                ...
                for (ii = 3; ii <= ido; ii += 2) {
                    ...
                    arg = fi * argld;
                    int idx = i + n;
                    wtable_r[idx - 2] = (double)Math.cos(arg);
                    wtable_r[idx - 1] = (double)Math.sin(arg);
                }
                is += ido;
            }
            l1 = l2;
    ...
    private void bluesteini() {
        ...
        for (int i = 1; i < n; i++) {
            ...
            if (k >= 2 * n)
                k -= 2 * n;
            arg = pi_n * k;
            bk1[2 * i] = (double)Math.cos(arg);
            bk1[2 * i + 1] = (double)Math.sin(arg);
        }
        double scale = (double)(1.0 / nBluestein);
        bk2[0] = bk1[0] * scale;
        bk2[1] = bk1[1] * scale;
    ...
    private void makewt(int nw) {
        ...
        if (nw > 2) {
            nwh = nw >> 1;
            delta = (double)(0.785398163397448278999490867136046290 / nwh);
            delta2 = delta * 2;
            wn4r = (double)Math.cos(delta * nwh);
            w[0] = 1;
            w[1] = wn4r;
            if (nwh == 4) {
                w[2] = (double)Math.cos(delta2);
                w[3] = (double)Math.sin(delta2);
            } else if (nwh > 4) {
                makeipt(nw);
                w[2] = (double)(0.5 / Math.cos(delta2));
                w[3] = (double)(0.5 / Math.cos(delta * 6));
                for (j = 4; j < nwh; j += 4) {
                    deltaj = delta * j;
                    deltaj3 = 3 * deltaj;
                    w[j] = (double)Math.cos(deltaj);
                    w[j + 1] = (double)Math.sin(deltaj);
                    w[j + 2] = (double)Math.cos(deltaj3);
                    w[j + 3] = (double)-Math.sin(deltaj3);
                }
            }
            nw0 = 0;
            while (nwh > 2) {
                ...
                if (nwh == 4) {
                    ...
                    wk1r = w[nw0 + 4];
                    wk3r = w[nw0 + 6];
                    w[nw1 + 2] = (double)(0.5 / wk1r);
                    w[nw1 + 3] = (double)(0.5 / wk3r);
                    for (j = 4; j < nwh; j += 4) {
                        int idx1 = nw0 + 2 * j;
                        int idx2 = nw1 + j;
                        wk1r = w[idx1];
    ...
    private void makect(int nc, double[] c, int startc) {
        ...
        if (nc > 1) {
            nch = nc >> 1;
            delta = (double)(0.785398163397448278999490867136046290 / nch);
            c[startc] = (double)Math.cos(delta * nch);
            c[startc + nch] = (double)(0.5 * c[startc]);
            for (j = 1; j < nch; j++) {
                deltaj = delta * j;
                c[startc + j] =  (double)(0.5 * Math.cos(deltaj));
                c[startc + nc - j] = (double)(0.5 * Math.sin(deltaj));
            }
        }
    }
    ...
    void radf3(final int ido, final int l1, final double in[], final int in_off, final double out[], final int out_off, final int offset) {
        final double taur = -0.5f;
        final double taui = 0.866025403784438707610604524234076962f;
        int i, ic;
    ...
    void radf4(final int ido, final int l1, final double in[], final int in_off, final double out[], final int out_off, final int offset) {
        final double hsqt2 = 0.707106781186547572737310929369414225f;
        int i, ic;
    ...
    void radb4(final int ido, final int l1, final double in[], final int in_off, final double out[], final int out_off, final int offset) {
        final double sqrt2 = 1.41421356237309514547462185873882845f;
        int i, ic;
    ...
    void radf5(final int ido, final int l1, final double in[], final int in_off, final double out[], final int out_off, final int offset) {
        final double tr11 = 0.309016994374947451262869435595348477f;
        final double ti11 = 0.951056516295153531181938433292089030f;
        final double tr12 = -0.809016994374947340240566973079694435f;
        final double ti12 = 0.587785252292473248125759255344746634f;
        int i, ic;
    ...
    void radfg(final int ido, final int ip, final int l1, final int idl1, final double in[], final int in_off, final double out[], final int out_off, final int offset) {
        ...
        dcp = (double)Math.cos(arg);
        dsp = (double)Math.sin(arg);
        ipph = (ip + 1) / 2;
        nbd = (ido - 1) / 2;
        if (ido != 1) {
    ...
    private void scale(final double m, final double[] a, int offa, boolean complex) {
        final double norm = (double)(1.0 / m);
        int n2;
        if (complex) {
            n2 = 2 * n;
        } else {
