...
public class MyWavFile
{
    ...
    //SIgona2017 private FileInputStream iStream;		// Input stream used for reading data
    ...
    // Sigona2018
    private double[] daSamples = null;
    public double [] getSamples() throws IOException, Exception {
    	if (null==daSamples)
    		readAll();
    	return daSamples;
    }
    private float[] faSamples = null; // float support
    public float [] getFloatSamples() throws IOException, Exception { // float support
    	if (null==faSamples)
    		readAllFloat();
    	return faSamples;
    }
    public void readAll() throws IOException, Exception {
		// Some audio properties
    	int iNumChannels = this.getNumChannels();
    	int iValidBits   = this.getValidBits();
    	int iNumSamples  = (int)this.getNumFrames();    	
    	double dSampleRate  = (double)this.getSampleRate();
    	// Read the whole file
    	this.daSamples = new double[iNumSamples];
    	this.readFrames(this.daSamples, iNumSamples);
    	// ... then close
    	this.close();
    }
    ...
    public int getBytesPerSample() { return bytesPerSample; }; //Sigona2017

    public static MyWavFile newWavFile(File file, int numChannels, long numFrames, int validBits, long sampleRate) throws IOException, Exception
    {
        // Instantiate new Wavfile and initialise
        MyWavFile wavFile = new MyWavFile();
        wavFile.file = file;
        ...
        if (validBits < 2 || validBits > 65535) throw new Exception("Illegal number of valid bits, valid range 2 to 65536");
    ...
    // Sigona2017 - Start
    public static MyWavFile openWavFile(File file) throws IOException, Exception
    {
        ...
        MyWavFile wavFile = openWavFile(iStream);
        wavFile.file = file;
        return wavFile;
    }

    //Sigona2017 public static MyWavFile openWavFile(FileInputStream inputStream) throws IOException, Exception
    public static MyWavFile openWavFile(InputStream inputStream) throws IOException, Exception
    {
        MyWavFile wavFile = new MyWavFile();
        wavFile.iStream = inputStream;
        int size = wavFile.iStream.available();
    // Sigona2017 - End
        ...
        /* Sigona2017
        if (file.length() != chunkSize+8) { //if (file.length() != chunkSize+8) {
            throw new Exception("Header chunk size (" + chunkSize + ") does not match file size (" + file.length() + ")");
        }*/
        if (size != chunkSize+8) { //if (file.length() != chunkSize+8) {
            throw new Exception("Header chunk size (" + chunkSize + ") does not match file size (" + size + ")");
        }
        ...
        while (true)
        {
            ...
            if (chunkID == FMT_CHUNK_ID)
            {
                ...
                if (wavFile.numChannels == 0) throw new Exception("Number of channels specified in header is equal to zero");
                if (wavFile.blockAlign == 0) throw new Exception("Block Align specified in header is equal to zero");
                if (wavFile.validBits < 2) throw new Exception("Valid Bits specified in header is less than 2");
                if (wavFile.validBits > 64) throw new Exception("Valid Bits specified in header is greater than 64, this is greater than a long can hold");
            ...
            else if (chunkID == DATA_CHUNK_ID)
            {
                ...
                if (chunkSize % wavFile.blockAlign != 0) throw new Exception("Data Chunk size is not multiple of Block Align");
    ...
    public int getByteSize() {
        return (int)(numFrames*blockAlign);
    }
    //Sigona2017
    public long readBytes(byte [] destBuf, int bytesToRead) throws IOException, Exception {
        if (ioState != IOState.READING) throw new IOException("Cannot read from WavFile instance");

        long bytesRead = iStream.read(destBuf, 0, bytesToRead);
    ...
    public int readFrames(int[] sampleBuffer, int offset, int numFramesToRead) throws IOException, Exception
    {
        if (ioState != IOState.READING) throw new IOException("Cannot read from WavFile instance");
    ...
    public int writeFrames(int[] sampleBuffer, int numFramesToWrite) throws IOException, Exception
    {
        return writeFrames(sampleBuffer, 0, numFramesToWrite);
    }

    public int writeFrames(int[] sampleBuffer, int offset, int numFramesToWrite) throws IOException, Exception
    {
    ...
    // Long
    // ----
    public int readFrames(long[] sampleBuffer, int numFramesToRead) throws IOException, Exception
    {
        return readFrames(sampleBuffer, 0, numFramesToRead);
    }

    public int readFrames(long[] sampleBuffer, int offset, int numFramesToRead) throws IOException, Exception
    {
        if (ioState != IOState.READING) throw new IOException("Cannot read from WavFile instance");
    ...
    public int writeFrames(long[] sampleBuffer, int numFramesToWrite) throws IOException, Exception
    {
        return writeFrames(sampleBuffer, 0, numFramesToWrite);
    }

    public int writeFrames(long[] sampleBuffer, int offset, int numFramesToWrite) throws IOException, Exception
    {
        if (ioState != IOState.WRITING) throw new IOException("Cannot write to WavFile instance");
    ...
    // Double
    // ------
    public int readFrames(double[] sampleBuffer, int numFramesToRead) throws IOException, Exception
    {
        return readFrames(sampleBuffer, 0, numFramesToRead);
    }
    ...
    //Sigona added october2018, float support: START
    
    public void readAllFloat() throws IOException, Exception {
		// Some audio properties
    	int iNumChannels = this.getNumChannels();
    	int iValidBits   = this.getValidBits();
    	int iNumSamples  = (int)this.getNumFrames();    	
    	double dSampleRate  = (double)this.getSampleRate();
    	// Read the whole file
    	this.faSamples = new float[iNumSamples];
    	this.readFrames(this.faSamples, iNumSamples);
    	// ... then close
    	this.close();
    }
    
    public int readFrames(float[] sampleBuffer, int offset, int numFramesToRead) throws IOException, Exception
    {
        if (ioState != IOState.READING) throw new IOException("Cannot read from WavFile instance");
    ...
    public int writeFrames(float[] sampleBuffer) throws IOException, Exception {
    	
    	int numFramesToWrite = sampleBuffer.length;
    	double [] daSampleBuffer = new double[numFramesToWrite];
    	for (int i=0; i<numFramesToWrite; i++)
    		daSampleBuffer[i] = (double)sampleBuffer[i];
    	
    	return writeFrames(daSampleBuffer, numFramesToWrite);
    }
    
    
    public int writeFrames(float[] sampleBuffer, int offset, int numFramesToWrite) throws IOException, Exception {
    	double [] daSampleBuffer = new double[numFramesToWrite];
    	for (int i=0; i<numFramesToWrite; i++)
    		daSampleBuffer[i] = (double)sampleBuffer[offset + i];
    	return writeFrames(daSampleBuffer, numFramesToWrite);
    }
  ...
  //Sigona added october2018, float support: END

    
    
    public int writeFrames(double[] sampleBuffer, int numFramesToWrite) throws IOException, Exception
    {
        return writeFrames(sampleBuffer, 0, numFramesToWrite);
    }

    public int writeFrames(double[] sampleBuffer, int offset, int numFramesToWrite) throws IOException, Exception
    {
        if (ioState != IOState.WRITING) throw new IOException("Cannot write to WavFile instance");
    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            ...
            {
                MyWavFile readWavFile = openWavFile(new File(filename));
                readWavFile.display();
                ...
                MyWavFile writeWavFile = newWavFile(new File("out.wav"), numChannels, numFrames, validBits, sampleRate);
...
//				long[] buffer = new long[BUF_SIZE * numChannels];
            ...
            MyWavFile writeWavFile = newWavFile(new File("out2.wav"), 1, 10, 23, 44100);
    ...
    /**
     * Save samples to a new file in a single operation
     * @throws Exception 
     * @throws IOException 
     * 
     */
    public static void saveMonoToFile(String outpuFilePath, float [] samplesFloat, long sampleRate, int validBits) throws IOException, Exception {
		
    	System.out.println("Saving noisy speech: " + outpuFilePath);
		MyWavFile oWriteWavFile = MyWavFile.newWavFile(new File(outpuFilePath), 1, samplesFloat.length, validBits, sampleRate);
		oWriteWavFile.writeFrames(samplesFloat);
		oWriteWavFile.close();
    }
}
