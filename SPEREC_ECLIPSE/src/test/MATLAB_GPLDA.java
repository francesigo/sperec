...
public class MATLAB_GPLDA {
	 ...
	 * @param g
	 */
	public MATLAB_GPLDA(GPLDA_Models g) {
	...
	public GPLDA_Models estimate(MyMatrix Data, Short [] spk_labs_, int niter) throws Exception {
		...
		//data = data(:, I)
		MyMatrix SortedData = Data.sortColumnsAndKeys(spk_labs,  MySort.SortDir.ASCENDING);
		...
		{ // Check for debug
			...
			TEST.compare_matrices("\nComparing SortedDataNormed: ", SortedDataNormed, res);
			res = null;
		}
		...
		//MyMatrix W1 = calc_white_mat_arg.calc_white_mat(this.debug); 		//W1   = calc_white_mat(cov(data'));
		MyMatrix W1 = TEST.test_calc_white_mat("\ntest_calc_white_mat for W1 (in GPLDA): ", SortedData.transpose().cov());
		...
		init(ndim);  // I placed the above instructions in the init() method

		Phi.minusEqualColumnVector(Phi.mean(2)); // Phi = bsxfun(@minus, Phi, mean(Phi, 2));
		
		MyMatrix Phitr_times_Phi = Phi.transpose().times(Phi);
		
		TEST.test_svd("\nsvd(Phitr_times_Phi)", Phitr_times_Phi); //, true);
		MyMatrix W2 = TEST.test_calc_white_mat("\nW2 = Phitr_times_Phi.calc_white_mat(), ", Phitr_times_Phi); //W2 = calc_white_mat(Phi' * Phi);

		Phi = Phi.times(W2); // Phi = Phi * W2; % orthogonalize Eigenvoices (columns)
		
		System.out.println("\nRe-estimating the Eigenvoice subspace with " + nphi + " factors ...\n");
		...
		for (int iter=0; iter<niter; iter++)
		{
			...
			if (debug) {
				...
				TEST.compare_matrices("Ey:\t", Ey, (double [][])res.get("Ey"));
				TEST.compare_matrices("Eyy:\t", Eyy, (double [][])res.get("Eyy"));
				res = null;
			}
														//	% maximization
	...
	private void expectation_plda(MyMatrix data, double [] spk_counts) throws Exception {
		...
		for (int ix = 0; ix<nuniq; ix++) { 							//for ix = 1 : nuniq
			...
			if (debug) {
				...
				TEST.compare_matrices("Cyy:\t", Cyy, res);
				res = null;
			}
...
% Microsoft Research, Conversational Systems Research Center
	...
	public static MATLAB_GPLDA readFromInputStream(InputStream is) throws IOException {
		
		GPLDA_Models g = GPLDA_Models.readFromInputStream(is);
		MATLAB_GPLDA gplda = new MATLAB_GPLDA(g);
	 ...
	 */
	public static MATLAB_GPLDA readFromFile(File file) throws IOException, ClassNotFoundException {
		...
		MATLAB_GPLDA gplda = MATLAB_GPLDA.readFromInputStream(is);
