...
import sperec_common.de.fau.cs.jstk.util.Arithmetics;
...
public final class MmieAccumulator {
	...
	public void addSegment(int target, 
		...
		System.out.println("addSegment: Kr=" + Kr); //logger.info("addSegment: Kr=" + Kr);
		...
		for (Map.Entry<Integer, Double> e : logscores.entrySet()) {
			...
			System.out.println("adding denominator with p=" + post); // logger.info
	...
	public static void MmieUpdate(HashMap<Integer, Mixture> invin, 
		...
		if (invout.size() > 0)
			System.out.println("The referenced out-inventory was not empty"); // logger.info
		invout.clear();
			...
			for (int i = 0; i < min.nd; ++i) {
				...
				for (it = 0; it < maxit; ++it) {
					if (EBWUpdate(i, D, flags, s, min.components[i], mout.components[i])) {
						...
						System.out.println(i + " success at it=" + it + " D=" + D + " committing with D*2"); //logger.info
				...
				if (it == maxit) {
					System.out.println(" reached maxit (unexpected) -- restoring original density"); //logger.warn
