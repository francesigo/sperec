...
*/
package sperec_common.de.fau.cs.jstk.stat;
...
import sperec_common.de.fau.cs.jstk.stat.Density.Flags;
import sperec_common.de.fau.cs.jstk.stat.MmieAccumulator.MmieOptions;
...
public final class MleMixtureAccumulator {
	...
	public static void MleUpdate(Mixture min, 
		...
		if (norm == 0.0) {
			System.out.println("No occupancy logged; aborting reestimation"); // logger.info
			return;
		}
	...
	public static void main(String [] args) throws Exception {
		// Sigona BasicConfigurator.configure();
	...
	private static void doacc(String [] args) throws Exception {
		...
		while ((l = br.readLine()) != null) {
			...
			if (mixtures.size() > 0 && m.fd != mixtures.get(0).fd) {
				System.out.println("Feature dimensions of Mixtures do not match!");  //logger.error
				System.exit(1);
			}
		...
		if (mixtures.size() == 0) {
			System.out.println("No mixtures loaded?!");// logger.error
			System.exit(1);
		}
		
		double [] x = new double [mixtures.get(0).fd];
		
		System.out.println("read " + mixtures.size() +  " mixtures"); // logger.info
		...
		while ((l = br.readLine()) != null) {
			...
			if (iof.length != 2) {
				System.out.println("skipping invalid line: " + l); // logger.error
				continue;
			}
			...
			System.out.println(iof[0] + ": " + data.size() + " samples"); // logger.info
			...
			for (int i = 0; i < mixtures.size(); ++i) {
				...
				a.write(fos);
				prot.write(" " + llh);
				System.out.println(i + " " + iof[0] + " average-logscore = " + (llh/data.size())); // logger.info
			}
		...
		for (int i = 0; i < globllh.length; ++i)
			globllh[i] /= nsamples;
		System.out.println("Finished accumulating " + nfiles + " files (" + nsamples + " samples) average logscores = " + Arrays.toString(globllh)); // logger.info
	}
	...
	private static void dommie(String [] args) throws Exception {
		...
		System.out.println("read " + inventory.size() + " mixtures"); // logger.info
		...
		System.out.println("read " + j + " MLE accumulators"); // logger.info
		...
		System.out.println("writing out mixtures"); // logger.info
		for (int i = 0; i < iolist.size(); ++i) {
