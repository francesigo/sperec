...
// Sigona import junit.framework.Assert;

// Sigona import org.apache.log4j.Logger;

import Jama.CholeskyDecomposition;
import Jama.Matrix;

// Sigona import static org.junit.Assert.assertEquals;
...
public final class DensityFull extends Density {
	// Sigona public static Logger logger = Logger.getLogger(DensityFull.class);
	...
	public void update() {
		...
		*/
		...
		// by hand
		if (!chol.isSPD()) {
			// Sigona logger.info("Density#" + id + ".update(): enforced diagonal covariance");
			k = 0;
			for (int i = 0; i < fd; ++i) {
				for (int j = 0; j <= i; ++j, ++k) {
					if (i == j)
	...
	public DensityFull marginalize(boolean [] keep) {
		...
		// Sigona assertEquals(d.cov.length, k);
